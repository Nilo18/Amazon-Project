...
export function getCurrentDate() {
    return dayjs() // Example: "February 20"
  }

// Promise lets us handle asynchronous code better
// They let us split code into separate steps
// And let us wait for one step to finish before going to the next step
// Resolve lets us decide when to go to the next step
// then() runs the next step
// new Promise((resolve) => {
//     console.log('Promise')
//     loadProducts(() => {
//         resolve()
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    cart.forEach((selectedProduct, index) => {
        ...
        const freeShippingDate = getCurrentDate().add(7, 'day') // The shipping date for free delivery is one week (today + 7 days)
        ...
        const minShippingDate = getCurrentDate().add(3, 'day') // The shipping date for 4.99 delivery are 4 days (today + 4 days)
        ...
        const maxShippingDate = getCurrentDate().add(1, 'day') // The shipping date for 9.99 delivery is one day (today + 1 day)
        ...
        if (selectedProduct?.deliveryOptionId) {
            if (selectedProduct.deliveryOptionId === '1') {
                deliveryDate.textContent = `Delivery date: ${chosenDelivery}`;
            } else if (selectedProduct.deliveryOptionId === '2') {
                deliveryDate.textContent = `Delivery date: ${chosenDelivery2}`;
            } else if (selectedProduct.deliveryOptionId === '3') {
                deliveryDate.textContent = `Delivery date: ${chosenDelivery3}`;
            }
        } else {
            // Default to free shipping if no previous selection exists
            deliveryDate.textContent = `Delivery date: ${chosenDelivery}`;
        }
